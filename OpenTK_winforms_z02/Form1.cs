...
namespace OpenTK_winforms_z02
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            // *** INIT LIGHT 1 ***
            light1 = new LightSource((int)LightName.Light1);
            light1.Enabled = false;


            light1.Ambient = new float[] { 0.2f, 0.2f, 0.2f, 1f };
            light1.Diffuse = new float[] { 1f, 1f, 1f, 1f };
            light1.Specular = new float[] { 0.5f, 0.5f, 0.5f, 1f };
            light1.Position = new float[] { 0f, 0f, 100f, 1f };    // deasupra cubului
        }
        ...
        private void GlControl1_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (statusMouseDown && lightON && light1.Enabled)
            {
                light1.Position[0] = e.X - (GlControl1.Width / 2);
                light1.Position[1] = (GlControl1.Height / 2) - e.Y;
                GlControl1.Invalidate();
            }
        }
        ...
        private void btnLight1_Click(object sender, EventArgs e)
        {
            ...
            btnLight1.Text = light1.Enabled ? "Sursa 1 ON" : "Sursa 1 OFF";
            GlControl1.Invalidate();
        }
        ...
        private void GlControl1_Paint(object sender, PaintEventArgs e)
        {
            ...
            Matrix4 perspective = Matrix4.CreatePerspectiveFieldOfView(camDepth,4f / 3f, 1f, 10000f);
        ...
        private void DeseneazaCubQ()
        {
            ...
            for (int i = 0; i < nQuadsList; i += 4)
            {
                // calcul normală la față
                int i0 = arrQuadsList[i];
                int i1 = arrQuadsList[i + 1];
                int i2 = arrQuadsList[i + 2];

                Vector3 v0 = new Vector3(arrVertex[i0, 0], arrVertex[i0, 1], arrVertex[i0, 2]);
                Vector3 v1 = new Vector3(arrVertex[i1, 0], arrVertex[i1, 1], arrVertex[i1, 2]);
                Vector3 v2 = new Vector3(arrVertex[i2, 0], arrVertex[i2, 1], arrVertex[i2, 2]);
                ...
                // cei 4 vârfuri ai feței
                for (int k = 0; k < 4; k++)
                {
                    GL.Color3(Color.White); // nu contează, ColorMaterial ia culoarea
                    int idx = arrQuadsList[i + k];
                    GL.Vertex3(arrVertex[idx, 0], arrVertex[idx, 1], arrVertex[idx, 2]);
                }
            }
            GL.End();
        ...
        private void DeseneazaCubT()
        {
            ...
            for (int i = 0; i < nTrianglesList; i += 3)
            {
                int i0 = arrTrianglesList[i];
                int i1 = arrTrianglesList[i + 1];
                int i2 = arrTrianglesList[i + 2];
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            ...
            if (lightON && light1 != null && light1.Enabled)
            {
                switch (keyData)
                {
                    case Keys.A:
                        light1.Position[0] -= speed;
                        break;
                    case Keys.D:
                        light1.Position[0] += speed;
                        break;
                    case Keys.W:
                        light1.Position[1] += speed;
                        break;
                    case Keys.S:
                        light1.Position[1] -= speed;
                        break;
                    case Keys.Q:
                        light1.Position[2] -= speed;
                        break;
                    case Keys.E:
                        light1.Position[2] += speed;
                        break;
...
// 1. Diferente intre iluminarea reala si cea din OpenGL:
// - In realitate lumina se reflecta peste tot si e mult mai complexa.
// - In OpenGL e un model simplificat, calculeaza doar direct lumina de la sursele definite.

// 2. Cate surse de lumina suporta OpenGL:
// - Suporta maxim 8 surse de lumina (Light0–Light7).

// 3. Ce este iluminarea de material:
// - Arata cum reactioneaza suprafata obiectului la lumina (ambient, diffuse, specular).
// - Se foloseste ca obiectele sa para facute din materiale diferite.

// 4. Efectul unei a doua surse de lumina:
// - Obiectele sunt luminate mai bine si se vad mai 3D.
// - Apar highlight-uri suplimentare si forma cubului se distinge mai clar.
